...
export default function DashboardPage() {
  ...
  const refreshApprovals = async () => {
    ...
    setPendingDocs(approvalRes.slice(0, 3) ?? []);
  ...
  const fetchDashboard = async () => {
    try {
      ...
      setRecentPosts(postRes.slice(0, 3) ?? []);
      setTeamAttendance(attendanceRes ?? []);
    } catch (error) {
    ...
    const client = new Client({
      ...
      onConnect: () => {
        ...
        client.subscribe("/user/queue/notifications/unread-count", (frame) => {
          ...
          setDashboard((prev) =>
            prev
              ? { ...prev, unreadNotificationCount: unreadCount ?? 0 }
              : prev,
          );
        });
      ...
      <div className={s.banner}>
        <div>
          ...
          <p className={s.bannerSub}>
            ...
            <strong>새 알림 {dashboard.unreadNotificationCount}건</strong>이
            있습니다.
          </p>
        </div>
              ...
              <div className={s.attendList}>
                {teamAttendance.map((a) => (
                  <div key={a.employeeId} className={s.attendRow}>
                    ...
                    <span
                      className={`${s.attendBadge} ${a.status === "ABSENT" ? s.attendAbsent : s.attendPresent}`}
                    >
              ...
              <div className={s.approvalItem}>
                <div className={s.approvalBody} style={{ width: "100%" }}>
                  ...
                  >
                    <div
                      style={{
                        width: `${(dashboard.todoTaskCount / (dashboard.todoTaskCount + dashboard.inProgressTaskCount + dashboard.doneTaskCount)) * 100}%`,
                        background: "#6B7280",
                        borderRadius: 999,
                        height: 6,
                      }}
              ...
              <div className={s.approvalItem}>
                <div className={s.approvalBody} style={{ width: "100%" }}>
                  ...
                  >
                    <div
                      style={{
                        width: `${(dashboard.inProgressTaskCount / (dashboard.todoTaskCount + dashboard.inProgressTaskCount + dashboard.doneTaskCount)) * 100}%`,
                        background: "#1A73E8",
                        borderRadius: 999,
                        height: 6,
                      }}
              ...
              <div className={s.approvalItem}>
                <div className={s.approvalBody} style={{ width: "100%" }}>
                  ...
                  >
                    <div
                      style={{
                        width: `${(dashboard.doneTaskCount / (dashboard.todoTaskCount + dashboard.inProgressTaskCount + dashboard.doneTaskCount)) * 100}%`,
                        background: "#059669",
                        borderRadius: 999,
                        height: 6,
                      }}
