...
import {
  ...
  Settings, FileCheck, ChevronRight, Zap,
  LayoutList, LogIn, Activity
} from "lucide-react";
...
const MAIN_NAV = [
  ...
  { path: "/approval", label: "전자결재", icon: FileCheck, id: "ws-apv", badge: 3 },
  { path: "/tasks", label: "업무", icon: CheckSquare, id: "ws-task" },
  { path: "/messenger", label: "메신저", icon: MessageSquare, id: "ws-msg", badge: 5 },
  { path: "/organization", label: "조직도", icon: Users, id: "ws-org" },
  { path: "/board", label: "게시판", icon: LayoutList, id: "ws-board" },
];

const BOTTOM_NAV = [
  { path: "/audit-log", label: "감사 로그", icon: Activity },
];
          ...
          {MAIN_NAV.map((item) => {
            ...
            return (
              ...
              >
                ...
                {active && !item.badge && <ChevronRight size={14} className={styles.chevron} />}
              </NavLink>
            );
          })}
        </div>
